    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-2">
          ...
          <p className="text-[10px] text-zinc-400 font-medium">د ترتیب بدلولو لپاره غشي وکاروئ</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {categories.map((cat, index) => {
            ...
            return (
              <div key={cat.id} className="flex items-center bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-2xl p-3 shadow-sm">
                <div className="flex items-center flex-1">
                  <div className="flex flex-col space-y-1 ml-2">
                    <button
                      onClick={() => moveCategory(index, 'up')}
                      disabled={index === 0}
                      className="p-1 text-zinc-400 hover:text-[var(--accent-color)] disabled:opacity-30 disabled:hover:text-zinc-400 transition-colors"
                    >
                      <ArrowUp size={14} />
                    </button>
                    <button
